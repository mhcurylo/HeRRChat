...
describe('broadcast_url', () => {
    ...
    describe('put source in space', () => {
        ...
        it('and deletes the user from the old room', () => {
            ...
            expect(store.getState().getIn(['spaces'])).to.equal(Map({
                '\/bb': Map({sources: Set.of()}),
                '\/dd': Map({sources: Set.of()}),
                '\/d1': Map({sources: Set.of('1234a')}),
                '\/d2': Map({sources: Set.of('1234a')}),
                '\/d3': Map({sources: Set.of('1234a')}),
                '\/d4': Map({sources: Set.of('1234a')}),
                '\/d5': Map({sources: Set.of('1234a')}) 
            }))
        })
    ...
    describe('delete source from space', () => {
        ...
        it('removes source and url, as in previous test.', () => {
            ...
            expect(store.getState().getIn(['spaces'])).to.equal(Map({
                '\/bb': Map({sources: Set.of()}),
                '\/dd': Map({sources: Set.of()}),
                '\/d1': Map({sources: Set.of()}),
                '\/d2': Map({sources: Set.of()}),
                '\/d3': Map({sources: Set.of()}),
                '\/d4': Map({sources: Set.of()}),
                '\/d5': Map({sources: Set.of('1234a')}) 
            }))
